...
//------------------------------------ class Image ------------------------------------------------
	...
	public:
		 ...
		 * \see setData
		 */ 
		Image(unsigned int width, unsigned int height, const std::vector<Color> data_ptr) : Array(width, height, data_ptr) {}
		
		/*! Copy constructor.
		 *
		 * \param src is the source image to replicate in this object.
		 */
		Image(const Image &src) : Array(src.getHeight(), src.getWidth(), src.buffer) {}
		...
		bool load(const std::string & filename, const std::string & format)
		{
			...
			float *curImage = imaging::ReadPPM(fn, (int*)&width, (int*)&height);

			if (curImage != nullptr) {
				cout << "\nImage dimensions are: " << this->width << " x " << this->height << endl << endl;
				this->buffer = std::vector<Color>(width * height);

				for (int i = 0; i < this->height; i++) {
					for (int j = 0; j < this->width; j++) {
						Color c(curImage[(i * this->width + j) * 3], curImage[(i * this->width + j) * 3 + 1], curImage[(i * this->width + j) * 3 + 2]);
						this->math::Array<Color>::setVal(j, i, c);
					}
				}
				delete[] curImage;
				return true;
		...
		*/
		...
		{
			if (format.compare(".ppm") != 0 || this->width == 0 || this->height == 0) {
				cout << "Unable to convert it!" << endl;
				return false;
			}

			float * newImage = new float[3 * this->getWidth() *  this->getHeight()];

			for (int i = 0; i < this->height; i++) {
				for (int j = 0; j < this->width; j++) {
					Color c =  this->math::Array<Color>::getVal(j, i);
					newImage[(i * this->width + j) * 3] = c[0];
					newImage[(i * this->width + j) * 3 + 1] = c[1];
					newImage[(i * this->width + j) * 3 + 2] = c[2];
				}
			}
			
			bool cs = imaging::WritePPM(newImage, this->width, this->height, filename.c_str());
			delete[] newImage;
			return cs;
		}
